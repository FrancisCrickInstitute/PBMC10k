---
title: "Pseudo-bulk Differential Gene Expression Analysis"
...
tableString <- paste0('An Excel table with the DGE results can be downloaded  ',tableLink,'.')
...
for (i in 1:length(DGEinputList)){
    ...
    redGroup <- as.vector(unlist(strsplit(label, "_vs_"))[1])
    blueGroup <- as.vector(unlist(strsplit(label, "_vs_"))[2])
    greyGroup <- "Rest"
    ...
    dfPlot$Cluster <- factor(dfPlot$Cluster, levels = dfCol$name)
...
for (i in 1:length(DGEtagVec)){
    ...
    dfPlot[["Y"]] <- dfPlot[,grep("contrast_1_logFC", names(dfPlot))]
    ...
    plotList[[tag]] <- ggplot(
      ...
      figureCount, 
                ':** ',
                ' MA-style plot for differential gene expression comparison ',gsub("_", " ", DGEtagVec),'. Download a pdf of this figure <a href="',FNrel,'" target="_blank">here</a>.',
                'An interactive version of this figure can be found ', link, '. ',
                tableLink
            )
...
for (i in 1:length(DGEtagVec)){
    ...
    plotList[[tag]] <- ggplot(
      ...
      figureCount, 
                ':** ',
                ' Volcano plot for differential gene expression comparison ',gsub("_", " ", DGEtagVec),'. Download a pdf of this figure <a href="',FNrel,'" target="_blank">here</a>.',
                'An interactive version of this figure can be found ', link, '. ',
                tableLink
            )
...
```{r documentation_saving, echo=TRUE, eval=TRUE, warning=FALSE, results="asis"} 

...
OsC@meta.data[["WT_IDH"]] <- ""
OsC@meta.data[grep("WT", OsC@meta.data$sampleID), "WT_IDH"] <- "WT"
OsC@meta.data[grep("IDH", OsC@meta.data$sampleID), "WT_IDH"] <- "IDH"

OsC@meta.data[["Gender"]] <- "Male"
OsC@meta.data[OsC@meta.data$sampleID %in% c("WT2", "WT4"), "Gender"] <- "Female"
# Non-proliferating Glia - Cluster (2, 4, 5)
# Proliferating Glia - Cluster (1, 6, 8)
# Early and late Progenitors - Cluster (0, 9)
# Neuronal progenitors - Cluster 7
# Remaining neurons from dissociation (but not happy cells) - Cluster 10
# Unknowns and/or not happy cells?- Cluster 3
...
comp_1 <- resList$DGEtable
OsC <- resList$OsC

tag <- paste0("DGE_", colName)

Obio@dataTableList[[tag]] <- data.frame(NULL)
Obio@dataTableList[[tag]] <- comp_1
...
colName <- "Male_vs_Female"

resList <- doDGEsc(
    obj = OsC,
    DGEselCol = "Gender",
    colName = colName,
    contrastTag = "contrast_2_",
    DGEsampleList = list(
    "Male" = "Male",
    "Female" = "Female"
  )
...
###############################################################################
## Third                                                                     ##
colName <- "WT_PC1_neg_vs_WT_PC1_pos"

OsC@meta.data[["WT_PC1"]] <- ""
OsC@meta.data[OsC@meta.data$WT_IDH == "WT" & OsC@meta.data$PC_1 > 0, "WT_PC1"] <- "WT_PC1_pos"
OsC@meta.data[OsC@meta.data$WT_IDH == "WT" & OsC@meta.data$PC_1 < 0, "WT_PC1"] <- "WT_PC1_neg"

resList <- doDGEsc(
    obj = OsC,
    DGEselCol = "WT_PC1",
    colName = colName,
    contrastTag = "contrast_3_",
    DGEsampleList =  list(
    "WT_PC1_neg" = "WT_PC1_neg",
    "WT_PC1_pos" = "WT_PC1_pos"
    )
)
...
baseFN <- "agl372A.DGE.table.xlsx"
outPutFN <- paste0(
  gsub("working/boeings/","", hpc.mount),
  "www/boeings/bioLOGIC_external/data/agl372A/html/report_tables/agl372A.DGE.table.xlsx"
)
...
database.table <- import.db.table.from.db(
    dbtable= "agl372A_sc_rna_seq_table",   
    dbname= "agl_data",   
    user= "boeingS",     
    password= db.pwd, 
    host= Obio@parameterList$host
...
Obio@parameterList$primDataDB <- "agl_data"

...
Obio@parameterList$primDataDB <- "csl_data"
...
#Obio@parameterList$host <- "10.27.241.234"
#Obio@parameterList$db.user <- "boeingS"

## External database credentials ##
# Obio@parameterList$host <- "www.biologic-db.org"
# Obio@parameterList$db.user <- "boeings"
...
FN <- "/camp/stp/babs/working/boeings/Projects/swantonc/dhruva.biswas/335_CSL_DB_scRNA_seq_lung_tumor_microenvironment_EMTAB6653/basedata/Allsamples.Cellview.Rds"
...
dfAdd <- dfAdd[row.names(dfAdd) %in% row.names(OsC@meta.data),]


#dfAdd[["cellID"]] <- row.names(dfAdd)
names(dfAdd) <- gsub("V1", "tSNE_1_article", names(dfAdd))
names(dfAdd) <- gsub("V2", "tSNE_2_article", names(dfAdd))
names(dfAdd) <- gsub("dbCluster", "Article_Cell_Type", names(dfAdd))
dfAdd <- dfAdd[,c("tSNE_1_article", "tSNE_2_article", "Article_Cell_Type")]
...
dfdbTable[["Gender"]] <- ""
dfdbTable[dfdbTable$Patient %in% c("P1", "P4"), "Gender"] <- "Female"
dfdbTable[dfdbTable$Patient %in% c("P2", "P3", "P5"), "Gender"] <- "Male"
...
upload.datatable.to.database(
    ...
    db.col.parameter.list = list(
        "VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_general_ci" = c("cellType","cellIdent","sample_","cellID","orig_ident","old_ident","clustIdent", "hmIdent","sampleID", "CellFromTumor", "Patient", "Region",  "Article_Cell_Type", "Gender"),
...
shinyProjectPath <-paste0(shinyBasePath, Obio@parameterList$project_id,"_app")
...
query7 <- paste0(
    "GRANT SELECT on ", Obio@parameterList$primDataDB,".",Obio@parameterList$PCAdbTableName, " TO ",sUser,"@'shiny-bioinformatics.crick.ac.uk';"
)
...
query8 <- paste0(
    "GRANT SELECT on ", Obio@parameterList$primDataDB,".",expDbTable, " TO ",sUser,"@'shiny-bioinformatics.crick.ac.uk';"
)
...
query9 <- paste0(
    "GRANT SELECT on ", Obio@parameterList$primDataDB,".",geneTb, " TO ",sUser,"@'shiny-bioinformatics.crick.ac.uk';"
)
...
## Create log-in file ##
url <- Obio@parameterList$host
id <- sUser
id2 <- sPwd
db <- Obio@parameterList$primDataDB
...
dfPercCellsExpr <- dfPercCellsExpr[dfPercCellsExpr$gene %in% Obio@dataTableList$referenceList$top100var, ]
...
cat("<html>");cat("\n");
cat("<head>");cat("\n");
catString <- paste0('<meta http-equiv="refresh" content="0; url=http://shiny.thecrick.org/babs/boeings/',Obio@parameterList$project_id,'_app/" />')
cat(catString);cat("\n");
cat("</head>");cat("\n");
cat("</html>");cat("\n");
...
names(dfAddCounts) <- gsub("contrast_1_logFC_", "contrast_A_logFC_bulkRNASeq_Park_et_al_", names(dfAddCounts) )
names(dfAddCounts) <- gsub("contrast_1_padj_", "contrast_A_padj_bulkRNASeq_Park_et_al_", names(dfAddCounts) )
names(dfAddCounts) <- gsub("contrast_1_lg10p_", "contrast_A_lg10p_bulkRNASeq_Park_et_al_", names(dfAddCounts) )

dfAddCounts <- dfAddCounts[dfAddCounts$hgnc_symbol %in% database.table$hgnc_symbol, ]
...
cmd.vec <- upload.datatable.to.database(
    ...
    db.col.parameter.list = list(
        ...
        "DECIMAL(6,3) NULL DEFAULT NULL" = c("avg_diff_Cluster","add_counts_",  "lg2BaseMean","lg2baseMean","contrast_P_PCA_estimatePCA","^CoVar$","NES", "logFC", "lg2_avg","norm_counts", "intensity", "^int", "iBAQ","^localization_prob$", "stat", "lg10p","contrast_x_"),
...
## Upload to database                                                        ##
###############################################################################
Obio@parameterList$cat.ref.db <- Obio@parameterList$primDataDB

---
title: "Integration Schaefer Lab single-nuclei data asl319 and Tepe et al 20k"
author: "Stefan Boeing"
lab: "Andreas Schaefer & Alex Fleischmann"
scientist: "Sara Zeppilli, Robin Alshire,  Tobias Ackles"
...
for (i in 1:length(reductionVec)){
    ...
    plotList[[tag]] <- DimPlot(OsC, reduction = reductionVec[i], label = TRUE, split.by = "sampleID")
...
This analyis gives an indication of which cells MIGHT be doublets.
...
#RidgePlot(OsC, features = c("TOP2A", "PCNA"), ncol = 2, log=T, slot="data")

Obio@dataTableList[["dfCellCycleIndex"]] <- OsC[[]]
...
if (nrow(OsC@meta.data) > 5000){
    set.seed(127)
    n.cells <- 5000
    OsC_HM <- SubsetData(OsC, cells.use = sample(x = object@cell.names, size = n.cells) )

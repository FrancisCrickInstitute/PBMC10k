...
names(dfExtra) <- gsub("^cluster$", "meta_Article_Cluster", names(dfExtra))
names(dfExtra) <- gsub("^CellType$", "meta_Article_Cell_Type", names(dfExtra))
names(dfExtra) <- gsub("^CellType$", "meta_Article_Cell_Type", names(dfExtra))
names(dfExtra) <- gsub("^CellType$", "meta_Article_Cell_Type", names(dfExtra))

dfExtra[["meta_Patient"]] <- sapply(dfExtra$Patient_piece, function(x) unlist(strsplit(x, "_"))[1])
dfExtra$meta_Patient <- paste0("P", dfExtra$meta_Patient)
...
#
names(dfExtra) <- gsub("CellFromTumor", "meta_CellFromTumor", names(dfExtra))
names(dfExtra) <- gsub("Annotation", "meta_Article_CellType_Annotation", names(dfExtra))
#
#
row.names(dfExtra) <- dfExtra$cell

selVec <- names(dfExtra)[grep("meta_", names(dfExtra))]
...
# FN <- "/camp/stp/babs/working/boeings/Projects/leys/joan.manils/383_SLL_JM_singleCellRNASeq_PatSkinDisease_EGAS00001002927/basedfExtra[["meta_Region"]] <- sapply(dfExtra$Patient_piece, function(x) unlist(strsplit(x, "_"))[2])
#data/publication_celldata_extraCol_selection.txt"
# 
# dfExtra <- read.delim(
#     FN,
#     header = T,
...
upload.datatable.to.database(
    ...
    db.col.parameter.list = list(
        "VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_general_ci" = c("cellType","cellIdent","sample_","cellID","orig_ident","old_ident","clustIdent", "hmIdent","sampleID", "clusterName",  "clusterColor", "sampleName", "sampleColor", "CellFromTumor", "Patient", "Region",  "Article_Cell_Type", "Gender","Glia_vs_Neuron", "NitChol_vs_Serotonergic", "Nitrergic_vs_Cholinergic", "Sample_Group", "CC_Difference", "meta_", "cluster_sample", "DGE_GliaT_vs_NeuronT", "^Cluster"),

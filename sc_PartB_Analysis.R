...
OsC@meta.data[["Mouse"]] <- OsC@meta.data$orig.ident
...
FNcol <- paste0(Obio@parameterList$localWorkDir, "/customClusterAnnotation.txt")
tableValid <- FALSE
...
if (!file.exists(FNcol) | !tableValid){
    ...
    FNcol <- paste0(Obio@parameterList$localWorkDir, "clusterAnnotation.txt")
    write.table(
        dfClusterAnnotation,
        FNcol,
        row.names=F,
...
FNsample <- paste0(Obio@parameterList$localWorkDir, "customSampleAnnotation.txt")
tableValid <- FALSE
...
dfPair <- unique(OsC@meta.data[,c("sampleID", "sampleColor")])
row.names(dfPair) <- dfPair$sampleID

dfPlot$Cluster <- factor(dfPlot$Cluster, levels = dfPair$sampleID)
...
if (length(grep("sampleID", names(OsC@meta.data))) > 0){
    cluster.averages <- AverageExpression(OsC, return.seurat = TRUE, add.ident = "sampleName")
...
### Heatmap and Dotplot for the top5 cluster defining genes {`r tabVar`}

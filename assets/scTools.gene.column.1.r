        ...
        for (i in 1:length(sampleNames)){
            if (obj@sampleDetailList[[sampleNames[i]]]$type == "TenX"){
                ...
                if (file.exists(rawFN)){
                    rawSampleList[[sampleNames[i]]] <- Read10X(data.dir = rawFN, gene.column=1)
                    filtSampleList[[sampleNames[i]]] <- Read10X(data.dir = baseFN, gene.column=1)
    ...
    def=function(
        ...
        for (i in 1:length(obj@sampleDetailList)){
            ...
            if ( obj@sampleDetailList[[sampleID]]$type == "loomFiles" ){
                ...
                assign(
                    "fullMat", #names(obj@parameterList[[obj@parameterList$inputMode]])[i],
                    Read10X(data.dir = dataDir, gene.column=1)
                )
...
setGeneric(
    ...
    for (i in 1:length(obj@sampleDetailList)){
        ...
        SampleList[[i]] <- RunPCA(
            SampleList[[i]], 
            npcs = Obio@sampleDetailList[[i]]$singleCellSeuratNpcs4PCA, verbose = FALSE
        )
        ## Do tSNE ##
        ...
        SampleList[[i]] <- FindClusters(SampleList[[i]], resolution = Obio@sampleDetailList[[i]]$singleCellClusterParameter)

...
for (i in 1:length(clusterVec)){
    ...
    link <- paste0('<a href="https://',urlString,'/',Obio@parameterList$project_id,'/scatterplot?x_axis=add_counts_Avg_log10_Expr_all&y_axis=norm_counts_Avg_log10_Expr_',clusterVec[i], '&cat_id=', catID,'" target="_blank">here</a>')
    
    heatmaplink <- paste0('In order to identify cluster specific marker genes a <a href="https://biologic.crick.ac.uk/vpl362A/category-view/', catID,'" target="_blank">cluster-maker gene heatmap for this gene set</a> might be interesting.')
    ...
    if ((length(posTestGeneSet) >= 3) |(length(negTestGeneSet) >= 3)){
        ...
        if ((nrow(dfNegEnriched) > 0) | (nrow(dfPosEnriched) > 0)){
            ...
            figLegend <- paste0(
               ...
               PCAdimensions[j],
                " associated with them. Download a pdf of this figure [here](", FNrel, "). To view these gene sets in the context of your data, go to ",link," and find these categories using the search box."
            )
            figureCount <- figureCount + 1 

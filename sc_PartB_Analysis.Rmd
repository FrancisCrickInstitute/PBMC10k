...
### Cell Type Plots {.tabset .tabset-fade .tabset-pills}
...
refdbname <- Obio@dbDetailList$ref.cat.db
dbtableList <- list(
...
for (i in 1:length(catEnrichmentList)){
    ...
    dfEnr$ID <- substr(dfEnr$ID, 1, 60)
    dfEnr$ID <- factor(dfEnr$ID, levels = unique(dfEnr$ID))
    tag <- paste0("Cell_Types_", names(catEnrichmentList)[i])
    
    plotList[[tag]] <- ggplot(
        data=dfEnr, aes(x= ID, y=lg10p, fill="#009900")
    ) + geom_bar(stat="identity", colour="black"
    ) + coord_flip() +scale_fill_manual(values=c("#009900"))  +  theme(
        axis.text.y   = element_text(size=8),
        axis.text.x   = element_text(size=8),
        axis.title.y  = element_text(size=8),
        axis.title.x  = element_text(size=8),
    ...
    ) + geom_hline(yintercept = c(-1*log10(0.05)), color = "red", size=0.5, lty=2
...
#                 clusterIDs[j],
#                 " as compared to all other clusters. Download a pdf of this figure <a href='",FNrel,"' target='_blank'>here</a>. To view these gene sets in the context of your data, go to [CategoryView > GO-BP](",link,") and find the above categories using the search box."
#             )
#             figureCount <- figureCount + 1 
#             
#             NewChnk <- paste0(

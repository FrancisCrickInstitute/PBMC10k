...
if (file.exists(FNcol)){
    ...
    inClust <- unique(as.vector(dfClusterAnnotation$seurat_clusters))
    oClust <- unique(as.vector(OsC@meta.data$seurat_clusters))
...
if (file.exists(FNsample)){
    ...
    if (length(pos1) != 0 & length(pos2) != 0 & length(pos3) != 0){
        ## Check if clusters match ##
        inClust <- unique(as.vector(dfClusterAnnotation$sampleID))
...
for (i in 1:length(Obio@parameterList$cat2DotplotList)){
    ...
    Idents(OsC) <- "clusterName"
        ...
        dfCol <- unique(OsC_DP@meta.data[,c("clusterName", "clusterColor")])
        colVec <- as.vector(dfCol$clusterColor)
...
if (length(geneVec) > 0){
    Obio@parameterList[["cat2HMplotList"]][["Top5_Cluster_Markers"]] <- geneVec 
}
...
if (length(geneVec) > 0){
    Obio@parameterList[["cat2HMplotList"]][["Top5_LR_Cluster_Markers"]] <- geneVec
}
...
geneVec <- as.vector(unique(dfTop5$gene))
if (length(geneVec) > 0){
    Obio@parameterList[["cat2HMplotList"]][["Top5_LR_TF_Cluster_Markers"]] <- geneVec
}

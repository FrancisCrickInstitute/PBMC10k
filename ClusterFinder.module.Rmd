...
## Rational: Run PCA on variable features, then scale data for heatmaps and other applications
    ...
    plotList[[tag]] <- DimPlot(OsC_CF, reduction = reductionVec[j], group.by = gsub("\\.", "_", paste0("integrated_snn_res.", clusterOptionVec[i])),label = TRUE, split.by = "sampleID")
    ...
    ## Color by cluster ##
    h <- sum(c("sampleName", "sampleColor") %in% names(OsC_CF@meta.data))
    if (h ==2){
      dfCol <- unique(OsC_CF@meta.data[,c("sampleName", "sampleColor")])
...
names(dfMeta) <- gsub("integrated_snn_res_", "ClusterTestRes_",names(dfMeta))
names(dfMeta) <- gsub("integrated_rnn_res_", "ClusterTestRes_",names(dfMeta))
